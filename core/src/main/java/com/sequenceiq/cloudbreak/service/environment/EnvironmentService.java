...
public class EnvironmentService extends AbstractWorkspaceAwareResourceService<Environment> {
    ...
    public DetailedEnvironmentResponse createForLoggedInUser(EnvironmentRequest request, @Nonnull Long workspaceId) {
        ...
        environment.setKdcConfigs(kerberosService.findByNamesInWorkspace(request.getKdcConfigs(), workspaceId));
    ...
    public DetailedEnvironmentResponse attachResources(String environmentName, EnvironmentAttachRequest request, Long workspaceId) {
        try {
            return transactionService.required(() -> {
                ...
                Set<KerberosConfig> kdcToAttach = kerberosService.findByNamesInWorkspace(request.getKdcConfigs(), workspaceId);
                if (validationResult.hasError()) {
                    throw new BadRequestException(validationResult.getFormattedErrors());
                }
                Environment environment = getByNameForWorkspaceId(environmentName, workspaceId);
    ...
    private ValidationResult validateAndDetachKdcConfigs(EnvironmentDetachRequest request, Environment environment, ValidationResult validationResult) {
        ...
        Map<KerberosConfig, Set<Cluster>> kdcsToClusters = kdcsToDetach.stream()
                .collect(Collectors.toMap(kdc -> kdc, kdc -> kerberosService.getClustersUsingResourceInEnvironment(kdc, environment.getId())));
